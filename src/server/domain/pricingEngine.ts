...
export function calculatePriceRisk(
  ...
  if (allLiveOffers.length >= 2 && allLiveOffers[0] === currentPriceEur) {
    ...
    if (secondCheapest > 0 && currentPriceEur < secondCheapest * 0.55) {
      // If the source has stable, self-consistent own history matching this price,
      // it is an established merchant price level rather than a sudden lone outlier
  ...
  if (sourceCheck.applicable && sourceCheck.isBreak) {
    ...
    if (hasCorroboratingPeer) {
      ...
      rawSeverity = Math.max(rawSeverity, 0.25);
    } else {
      // Lone break from established history without live peer corroboration
