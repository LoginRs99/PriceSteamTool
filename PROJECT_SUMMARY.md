...
- **Backend:** Node.js 22+, Express, TypeScript, Better-SQLite3 (WAL mode, prepared statement caching).
- **Frontend:** React 18, TypeScript, Vite, Vanilla CSS (Tailwind-free custom design system).
...
- **Engines:**
  ...
  - **Anomaly & Risk Engine:** Multi-factor pricing glitch detection (`LONE_BOTTOM_OUTLIER`, `SUB_EURO_PREMIUM_GLITCH`) with capped median outliers.
  - **Action Signal Engine:** `BUY_NOW`, `STRONG_BUY`, `FAIR_DEAL`, `WAIT`, `MONITOR`.
  - **Adaptive Keyshop Pacing:** Self-tuning exponential backoff (24h -> 336h) on stable prices with active target price override, 7s delay / 4s jitter, User-Agent rotation, and selective Client Hints.
...
npm test            # Run all 18 Vitest suites (186 tests)
