...
- **APIs:** Legacy SPA routes (`/api/*`) + New Anti-Rate-Limit REST API (`/api/v1/*`) with `POST /api/v1/offers/batch`, `POST /api/v1/games/resolve`, `GET /api/export/offers.csv`, ETag `304` caching, and IETF rate limit headers.
...
- **Engines:**
  ...
  - **Anomaly & Risk Engine:** Multi-factor pricing glitch detection with peer corroboration (`SUB_EURO_PREMIUM_GLITCH_CORROBORATED`, `LONE_BOTTOM_OUTLIER`), 72h active window filtering, and grouped review UI with 1-click game dismissal.
  - **Action Signal Engine:** `BUY_NOW`, `STRONG_BUY`, `FAIR_DEAL`, `WAIT`, `MONITOR`.
  - **Adaptive Keyshop Pacing:** Fair round-robin due-game sorting, sane 30-game default cap, exponential-tailed jitter, 5–15m natural hesitation pauses, self-tuning exponential backoff (24h -> 168h ceiling) on stable prices with active target price override, 7s delay / 4s jitter, User-Agent rotation, and selective Client Hints.
...
npm test            # Run all 20 Vitest suites (204 tests)
